...
[Note: Please review [R Basics Tutorial](R_basics_2_data_and_functions.md) if you are uncertain about objects, values, and functions.]
...
```{r select, exercise.setup='setupgun'}
...
```{r selectrange, exercise.setup='setupgun'}
...
```{r drop, exercise.setup='setupgun'}
select(gunpolls, -URL)
```
...
```{r rename, exercise.setup='setupgun'}
...
```{r selectrename, exercise.setup='setupgun'}
...
```{r assign, exercise.setup='setupgun'}
...
```{r overwrite, exercise.setup='setupgun'}
...
and select only the Question, Pollster, and republican/democratic support columns. 
Assign the result to the `gunpolls` object


```{r gunpollsex, eval=FALSE}
url <- "https://raw.githubusercontent.com/fivethirtyeight/data/master/poll-quiz-guns/guns-polls.csv"
gunpolls <- read_csv(url)
___
...
```{r filter, exercise.setup='setupgun3'}
...
```{r filter-boolean, exercise.setup='setupgun3'}
...
```{r isna, exercise.setup='readgunpollsdirty'}
...
```{r naomit, exercise.setup='readgunpollsdirty'}
...
```{r mutate, exercise.setup='setupgun2'}
...
```{r mutatex, exercise.setup='setupgun2', eval=F}
